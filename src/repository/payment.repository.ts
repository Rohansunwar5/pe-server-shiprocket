...
export interface CreatePaymentParams {
  orderId: string;
  orderNumber: string;
  user?: string; // Optional now for guest orders
  guestInfo?: {
    firstName: string;
    lastName?: string;
    email: string;
...
export class PaymentRepository {
  ...
  // Get payment by guest email and order number
  async getPaymentByGuestEmailAndOrderNumber(email: string, orderNumber: string) {
  ...
  async updatePaymentByOrderId(orderId: string, params: UpdatePaymentParams) {
    return this._model.findOneAndUpdate({ orderId }, params, { new: true });
  }

  async updatePaymentByOrderNumber(orderNumber: string, params: UpdatePaymentParams) {
    return this._model.findOneAndUpdate({ orderNumber }, params, { new: true });
  }
  ...
  // Get payments by guest email
  async getPaymentsByGuestEmail(email: string, page: number = 1, limit: number = 10) {
    const skip = (page - 1) * limit;
  ...
  async getPaymentStats(userId?: string, guestEmail?: string) {
    ...
    if (userId) {
      ...
      match["guestInfo.email"] = guestEmail;
    }
      ...
      { $match: match },
      {
        $group: {
          ...
          partialRefundedPayments: {
            $sum: {
              $cond: [{ $eq: ["$status", IPaymentStatus.PARTIAL_REFUNDED] }, 1, 0],
            },
          },
          successfulAmount: {
            $sum: {
              $cond: [{ $eq: ["$status", IPaymentStatus.CAPTURED] }, "$amount", 0],
            },
          },
          refundedAmount: {
            $sum: {
              $cond: [
                {
                  $in: [
                    "$status",
                    [IPaymentStatus.REFUNDED, IPaymentStatus.PARTIAL_REFUNDED],
                  ],
                },
                {
                  $sum: "$refunds.amount",
  ...
  async getPaymentsByMethodStats(userId?: string, guestEmail?: string) {
    const match: FilterQuery<IPayment> = {};
    if (userId) {
      match.user = userId;
    } else if (guestEmail) {
